...
//   .scale(100)
//   .translate([ 500, 250, 100 / 2 ])
//   .rotate([20,50,-40])

const projection = geoMercator().scale(150).translate([500, 250, 50]).rotate([20, 50, -40]);
...
const LimitesProvinciales = () => {
	...
	return (
		<svg className='provinces-map' viewBox='400 140 100 750'>
			<g className='countries'>
				{geographies.map((d, i) => (
					<path key={`path-${i}`} d={geoPath().projection(projection)(d)} className={styles.country} fill={`rgba(38,50,56,${1 /* (1 / geographies.length) * i */})`} stroke='#FFFFFF' strokeWidth={0.3} onClick={() => handleCountryClick(i)} />
				))}
			</g>
			{/* <g className="markers">
        {
...
export default LimitesProvinciales;

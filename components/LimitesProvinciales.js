...
const LimitesProvinciales = () => {
	...
	return (
		<svg className='provinces-map'>
			<g className='countries'>
				{geographies.map((d, i) => (
					<path key={`path-${i}`} d={geoPath().projection(projection)(d)} fill={`rgba(38,50,56,${1})`} stroke='#FFFFFF' strokeWidth={0.3} onClick={() => handleCountryClick(i)} />
				))}
			</g>
		</svg>
	);
